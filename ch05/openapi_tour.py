...
def main():
  ...
  if (natName == ''):
    ...
    # 파일 저장 : json 파일
    with open('./data/%s_%s, %d_%s.json' % (natName, ed, nStartYear, dataEND), 'w', encoding='utf-8') as outfile:
      jsonFile = json.dumps(jsonResult, indent=4, sort_keys=True, ensure_ascii=False)
      outfile.write(jsonFile)
    # 파일 저장 : ccsv 필요
    columns = ['입국자국가', '국가코드', '입국년월', '입국자 수']
    result_df = pd.DataFrame(result, columns=columns)
    result_df.to_csv('./data/%s_%s5, %d_%s.csv' % (natName, ed, nStartYear, dataEND), index=False, encoding='cp949')  # utf-8 아님!! 주의 cp949
