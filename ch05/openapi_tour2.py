...
def main() :
    ...
    result = []
    natName = ''
    print('<<국내 입출국한 외국인의 통계 테이터를 수집합니다.>>')
    nat_cd = input('국가 코드를 입력하세요(중국: 112 / 일본: 130 / 미국: 275) : ')
    nStartYear = int(input('데이터를 몇 년부터 수집할까요? : '))
    nEndYear = int(input('데이터를 몇 년까지 수집할까요? : '))
    # E : 방한외래관광객, D : 해외출국
    ed_cd = input('방한인지 해외출국인지 입력하세요(방한외래관광객: E / 해외출국: D) : ')
    ...
    if natName == '' :
        ...
        with open('./data/%s_%s_%d_%s.json' % (natName, ed, nStartYear, dataEND), 'w',
        ...
        if ed_cd == 'E' :
            ...
            columns = ['출국자국가', '국가코드', '출국년월', '출국자 수']
        result_df = pd.DataFrame(result, columns= columns)
        result_df.to_csv('./data/%s_%s_%d_%s.csv' % (natName, ed , nStartYear, dataEND),
